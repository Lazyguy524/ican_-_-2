...
from preprocess_bmcv import PreProcess
from postprocess_numpy import PostProcess
from sophon_inference import SophonInference
from utils import draw_bmcv, draw_numpy, is_img,draw_numpy_json
...
class YOLOv5:
    ...
    def predict(self, tensor):
        ...
        # print("Inference time use:{:.2f} ms".format((end_time-start_time)*1000)
        if not self.resort:
    ...
    if opt.is_video:
        ...
        if decoder.is_opened():
            ...
            while True:
                ...
                # save_name = '{}.jpg'.format(os.path.join(opt.output_dir, save_basename.replace('.jpg', '')))
        ...
        if batch_size != 1:
            raise ValueError(
                'bmodel batch size must be 1 in camera inference, but got {}'.format(
                    batch_size)
            )
        # decode
        decoder = sail.Decoder(input_path, True, opt.dev_id)
        if decoder.is_opened():
            ...
            while True:
                ...
                if frame_counter % skip_frames == 0:
                    ...
                    vis_image = draw_numpy(image_numpy, det[:,:4],
                                        masks=None, classes_ids=det[:, -1], conf_scores=det[:, -2])
                    encoded_image = cv2.imencode('.jpg', vis_image)[1]
        ...
        for ino in range(img_num):
            ...
            if batch_size == 1:
                ...
                vis_image = draw_numpy_json(image_numpy, det[:,:4],
                                       masks=None, classes_ids=det[:, -1], conf_scores=det[:, -2],file_name=os.path.basename(inp_batch[0]),output_dir=opt.output_dir)
                # save_basename = 'res_bmcv_{}'.format(os.path.basename(inp_batch[0]))
                # save_name = os.path.join(opt.output_dir, save_basename.replace('.jpg', ''))
                cv2.imencode('.jpg', vis_image)[1].tofile('{}.jpg'.format(save_name))
...
def parse_opt():
    ...
    parser.add_argument('--model', type=str, default = "yolov5n_1684x_f16.bmodel",help='bmodel path')
    parser.add_argument('--dev_id', type=int, default=0, help='device id')
    image_path = os.path.join(os.path.dirname(__file__),"corre_test")
    # image_path = os.path.join(os.path.dirname(__file__),"corre_test/test_4.jpeg")
    # image_path = os.path.join(os.path.dirname(__file__),"data_test/test.mp4")
    ...
    parser.add_argument('--output_dir', type=str, default="yolov5n_save", help='output image directory')
    parser.add_argument('--is_cam', default=0, type=int, help="input is camera?")
