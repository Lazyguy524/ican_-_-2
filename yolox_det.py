...
class Detector(object):
    ...
    def inference(self,input_tensor):
        ...
        end_time = time.time()
        print("Inference time use:{:.2f} ms , avg fps:{:.1f}".format((end_time-start_time)*1000,1/(end_time-start_time)))
...
if __name__ == "__main__":
    ...
    parse.add_argument('--file_name',default="../corre_test",type=str,help="video name or image_path")
    # parse.add_argument('--file_name',default="/dev/video0",type=str,help="video name or image_path")
    parse.add_argument('--bmodel_path',default="yolox_nano_1684x_f16.bmodel",type=str)
    parse.add_argument('--device_id',default=0,type=int)
    parse.add_argument('--detect_threshold',default=0.5,type=float,required=False)
    parse.add_argument('--nms_threshold',default=0.5,type=float, required=False)
    parse.add_argument('--save_path', default='yolox_nano_save', type=str)
    parse.add_argument('--is_cam',default=0,type=int,help="input is camera?")
            ...
            if batch_size == 1:
                ...
                if dete_boxs is not None:
                    ...
                    for dete_box in dete_boxs:
                        bmcv.rectangle(ost_image, int(dete_box[0]), int(dete_box[1]), 
                            int(dete_box[2]-dete_box[0]), int(dete_box[3]-dete_box[1]), (0, 0, 255), 4)
                        # cv2.putText(ost_image, dete_box[5] + ':' + str(round(dete_box[4], 2)),
                        # (int(dete_box[0]), int(dete_box[1]) - 5),
                        # cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), thickness=2)
                        bmcv.putText(ost_image,str(My_CLASSES[int(dete_box[5])]) + ':' + str(round(dete_box[4],2)),int(dete_box[0]), int(dete_box[1]) - 5,(0, 255, 0), 1,1) 
                img_bgr = sail.BMImage(handle, ost_image.height(), ost_image.width(),sail.FORMAT_BGR_PLANAR, sail.DATA_TYPE_EXT_1N_BYTE)
                bmcv.convert_format(ost_image,img_bgr)
                image_tensor = bmcv.bm_image_to_tensor(img_bgr)
                image_chw_numpy = image_tensor.asnumpy()[0]
                image_numpy = np.transpose(image_chw_numpy, [1, 2, 0]).copy()
                # print(image_tensor)
                cv2.imshow('Detection Result', image_numpy)
